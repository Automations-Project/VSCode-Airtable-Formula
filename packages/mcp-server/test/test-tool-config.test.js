...
describe('TOOL_CATEGORIES', () => {
  ...
  it('has correct read tools', () => {
    ...
    assert.deepEqual(readTools.sort(), [
      'download_formula_field', 'get_base_schema', 'get_table_schema', 'get_view', 'list_fields', 'list_record_templates', 'list_tables', 'list_view_sections', 'list_views', 'validate_formula',
    ]);
  });
...
describe('ToolConfigManager', () => {
  ...
  describe('default state', () => {
    ...
    it('enables all 62 tools on full profile', () => {
      const enabled = mgr.enabledToolNames();
      assert.equal(enabled.size, 62);
    });
  ...
  describe('enabledToolNames() for each profile', () => {
    it('read-only enables only 10 read tools', async () => {
    ...
    it('full enables all 62 tools', async () => {
  ...
  describe('getToolStatus()', () => {
    it('returns status for all 62 tools', async () => {
      await mgr.switchProfile('full');
      const status = mgr.getToolStatus();
      assert.equal(status.length, 62);
